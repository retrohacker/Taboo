...
class Indexer extends EventEmitter {
  constructor (...opts) {
    ...
    this.HashesProcessing = 0
    this.Lookup = 0
    this.Dropped = 0
    this.Queue = this.DiscoveryTable.keys()
    ...
    setInterval(() => {
      ...
      process.stdout.write(`Queue: \x1b[1m\x1b[33m${this.DiscoveryTable.size - this.HashesProcessed}\x1b[0m `)
      ...
      process.stdout.write(`Timeouts: \x1b[1m\x1b[33m${this.HashesProcessed - this.TorrentClient.torrents.length - this.TorrentTable.size}\x1b[0m `)
  ...
  _queue() {
    if (this.DiscoveryTable.size === this.HashesProcessed) {
      return
    }
    if (this.HashesProcessing === 150) {
      return
    }
    const hash = this.Queue.next().value
    this.HashesProcessed += 1
